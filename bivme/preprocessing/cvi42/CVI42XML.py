...
class CVI42XML:
    ...
    def __init__(
        ...
        self.contour = Contours.Contours(log=log)
        previous_item = []

        # CVI42 extracts all sorts of contours.

        # Extract contours
        errorCode = self.search_contours(filepath_contours, previous_item)
        ...
        for dir_name, subdir_list, file_list in os.walk(dcm_path):
            ...
            for filename in file_list:
                if dicom_extension in filename:  # check whether the file's DICOM
                    ...
                    try:
                        ...
                        position.append(np.asfarray(dicom_data.ImagePositionPatient))
                    ...
                    if (file_uid in dcm_uids) and (file_uid not in file_uids):
            ...
            if len(unique_snb != len(series_number)):
                ...
                for serie in unique_snb:
                    ...
                    slice_len = int(
                        np.sum(
                            np.sum(np.equal(serie_position, serie_position[0]), axis=1)
                            == 3
                        )
                    )
    ...
    def search_metadata(self, dcm_path, dicom_extension=".dcm"):
        # create an empty list
        dicom_files, files_frames = self.search_dicom_files(dcm_path, dicom_extension)

        for dcm_uid in dicom_files.keys():
            dicom_data = dcm.read_file(dicom_files[dcm_uid])

            image_position = np.array(
                [float(x) for x in dicom_data.ImagePositionPatient]
            )
            image_orientation = np.array(
                [float(x) for x in dicom_data.ImageOrientationPatient]
            )
            pixel_spacing = np.array([float(x) for x in dicom_data.PixelSpacing])
        ...
        if not (
            len(self.contour.list_frame_uids())
    ...
    def seart_txt_metadata(self, file_name):
        ...
        try:
            ...
            index_imPos = (
                np.where(["ImagePositionPatient" in x for x in info])[0][0] + 1
            )
            index_imOr = (
                np.where(["ImageOrientationPatient" in x for x in info])[0][0] + 1
            )
            timeFrame_index = int(np.where(["timeFrame" in x for x in info])[0][0] + 1)
            index_pixel_spacing = (
                np.where(["PixelSpacing" in x for x in info])[0][0] + 1
            )
        except:
        ...
        for dcm_uid in self.contour.list_sop_instance_uid():
            ...
            image_orientation = np.array(
                [float(x) for x in lines[dcm_uid][index_imOr : index_imOr + 6]]
            )
            pixel_spacing = np.array(
                [
                    float(x)
                    for x in lines[dcm_uid][
    ...
    def search_contours(self, root, parent_item):
        ...
        self.get_points(root, parent_item)
    ...
    def export_dicom_metadata(self, file_name, time_frame=None):
        ...
        for phase in time_frame:
            for frame_uid in self.contour.list_frame_uids_at_timeframe(phase):
                ...
                out.write(
                    frame_uid
                    + "\tframeID:\t{0}\ttimeFrame\t{1}".format(frame_id, phase)
                    + "\tImagePositionPatient\t{0:.3f}\t{1:.3f}\t{"
                    "2:.3f}\t".format(position[0], position[1], position[2])
