...
case = "RV01"
output_path = Path(
    r"R:\resmed201900006-biomechanics-in-heart-disease\Sandboxes\Debbie\collaborations\chicago-rv-mesh\analysis\gpfiles-raw",
    f"{case}",
).as_posix()
...
dcm_path = Path(
    r"R:\resmed201900006-biomechanics-in-heart-disease\Sandboxes\Debbie\collaborations\chicago-rv-mesh\images\chicago-cmr",
    f"{case}",
)  #'/dicom_metadata.txt'

# path to cvi42 file
contour_file = Path(
    r"R:\resmed201900006-biomechanics-in-heart-disease\Sandboxes\Debbie\collaborations\chicago-rv-mesh\analysis\wsx",
    f"{case}.cvi42wsx",
).as_posix()
...
if plot_contours:
    ...
    for index, points in enumerate(points_to_plot):
        cont_fig.plot_points(
            contours_types[index], points, color=cmap(norm(2 * index))[:3], size=1.5
        )
