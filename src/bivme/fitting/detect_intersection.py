...
def fix_intersection(case_name: str, config: dict, model_file: os.PathLike, output_folder: os.PathLike, biv_model_folder: os.PathLike = MODEL_RESOURCE_DIR) -> None:
    ...
    if bool(inter):
        ...
        my_logger.info(f"Intersections detected for case {os.path.basename(os.path.normpath(model_file))}")
        ...
        my_logger.info(f"Refitting of {str(case)}")
...
#
        ...
        my_logger.info("No intersection detected - moving on")
        return
    #print("inter", bool(inter))
...
if __name__ == "__main__":
    ...
    folders = [p.name for p in Path(config["output"]["output_directory"]).glob(args.patterns) if os.path.isdir(p)]
    ...
    try:
        for i, folder in enumerate(folders):
            ...
            models = [config["output"]["output_directory"] / folder / Path(name) for name in os.listdir(config["output"]["output_directory"] / folder) if rule.match(name)]
